...
1. Markov Chains
2. Rosenbluth Metropolis Hastings
...
our technique. The most ubiquitous of these methods is called ![Markov chain Monte Carlo](https://en.wikipedia.org/wiki/Markov_chain_Monte_Carlo).
...
### Rosenbluth-Metropolis-Hastings

Rosenbluth-Metropolis-Hastings enforces the reversibility constraint using 
...
Therefore, if we draw samples using the Rosenbluth-Metropolis-Hastings
algorithm, we draw samples from the target distribution $p(x)$.

## Coding Rosenbluth-Metropolis-Hastings in practice
...
the probability of the [data](https://github.com/dtu-qmcm/bayesian_statistics_for_computational_biology/main/data/metropolis_hastings_example.csv).
